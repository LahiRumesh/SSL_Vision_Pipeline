...
def create_unique_id(df,image_folder):
...
def convert_Input_CSV_to_yolo(vott_df,labeldict=dict(zip(['Object'],[0,])),
                            path='',train_name='train.txt',val_name='val.txt',
                            abs_path=False,val_split=0.1,target_name = 'data_file_out.txt'):
    ...
    train_set = dataset_copy.sample(frac=1-val_split, random_state=0)
...
def csv_data_process(folder_path,
                    val_split=0.1,
                    folder_name='train',class_names='class.names',
